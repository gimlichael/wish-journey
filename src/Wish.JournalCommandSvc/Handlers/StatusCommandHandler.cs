...
namespace Wish.JournalCommandSvc.Handlers
{
	public class StatusCommandHandler : CommandHandler
	{
		...
		{
			handlers.RegisterAsync<CreateStatus>(command => _commandQueue.SendAsync(command.ToMessage($"urn:status:id:{command.CorrelationId:N}".ToUri(), "journalcommandsvc.status.create").Yield()));
			handlers.RegisterAsync<UpdateStatus>(command => _commandQueue.SendAsync(command.ToMessage($"urn:status:id:{command.CorrelationId:N}".ToUri(), "journalcommandsvc.status.update").Yield()));
		}
	}
}
